...
<div class="col-lg-8 middle">
    ...
    <button  id="send" class="btn btn-success center-block">Submit</button>
</div>
<div class="col-lg-2 side"</div>
</body>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click','#send',function(){
            ...
            $('.question').each(function(){
                ...
                switch ($(this).attr('type')){
                    ...
                    case 'level':
                        ...
                        $.post('addResponsesParticipate.php',{questionId : questionId, responseId: value, other : undefined,id :id});
                        break;
                    case 'free':
                        var value = $(this).find('textarea').val();
                        $.post('addResponsesParticipate.php',{questionId : questionId, responseId: undefined, other : value,id :id });
                        break;
                    case 'simple':
                        var value = $(this).find('input').filter(':checked').val();
                        if(value == ''){
                            var other = $(this).find('#other_value').val();
                            $.post('addResponsesParticipate.php',{questionId : questionId, responseId: undefined, other: other,id :id});
                        }
                        else {
                            $.post('addResponsesParticipate.php',{questionId : questionId, responseId: value, other : undefined,id :id});
                        }
            ...
            $.post('setParticipation.php',{id : id}, function(data){
                if(data){
                    window.history.back();
                }
            });
